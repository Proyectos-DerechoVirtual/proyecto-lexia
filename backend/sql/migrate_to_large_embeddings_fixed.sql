...
RENAME TO document_embeddings_backup_20250820;
...
-- 3. Eliminar índices existentes si existen
DROP INDEX IF EXISTS idx_document_embeddings_law_name;
DROP INDEX IF EXISTS idx_document_embeddings_article_number;
DROP INDEX IF EXISTS idx_document_embeddings_category;
DROP INDEX IF EXISTS idx_document_embeddings_document_type;
DROP INDEX IF EXISTS idx_document_embeddings_topic_number;
DROP INDEX IF EXISTS idx_document_embeddings_difficulty;
DROP INDEX IF EXISTS idx_document_embeddings_exam_relevance;
DROP INDEX IF EXISTS idx_document_embeddings_keywords;
DROP INDEX IF EXISTS idx_document_embeddings_legal_refs;
DROP INDEX IF EXISTS idx_document_embeddings_type_law;

-- 4. Crear índices SOLO para metadatos (no para embeddings)
...
-- 5. Eliminar funciones antiguas si existen
...
DROP FUNCTION IF EXISTS search_documents(vector(3072), float, int, text, text, int, text, int);

-- 6. Función de búsqueda actualizada para 3072 dimensiones
...
-- 7. Función para búsqueda por keywords
...
-- 8. Función para búsqueda por referencias legales
...
DROP POLICY IF EXISTS "Allow service role to insert document_embeddings" ON public.document_embeddings;
DROP POLICY IF EXISTS "Allow service role to update document_embeddings" ON public.document_embeddings;
DROP POLICY IF EXISTS "Allow service role to delete document_embeddings" ON public.document_embeddings;
...
-- 12. Función helper para obtener estadísticas rápidas
...
-- DROP TABLE IF EXISTS document_embeddings_backup_20250820;